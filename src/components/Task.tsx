  ...
  import { ClockIcon, DateTimePicker, LocalizationProvider } from '@mui/x-date-pickers';
...
import { useAuth } from './AuthProvider';
  ...
  const Task: React.FC<TaskProps> = ({ task, index, selectedBoard, setSelectedBoard }) => {
    ...
    const sendEmailToUser = async (userId: string, boardId: string, email: string) => {
      await axios.post(`/user/${userId}/assign/${boardId}`, {
        email: email,
    ...
    const updateTask = async (title: string, description: string | null, dueDate: Date | null, points: number | null, assignee: string | null | undefined) => {
      ...
      await axios.put(`/tasks/${task._id}`, {
        ...
        const updatedTask = res.data.data;
          const updatedTasks = selectedBoard?.tasks.map(t => t._id === updatedTask._id ? updatedTask : t);
  ...
  const handleDeleteTask = async () => {
    ...
    await axios.delete(`/tasks/${task._id}/${boardId}`)
      .then((res) => {
        ...
        const deletedTaskId = res.data.data._id;
        const updatedTasks = selectedBoard?.tasks.filter(t => t._id !== deletedTaskId);
        const updatedColumns = selectedBoard?.columns.map(col => ({
          ...col,
          tasks: col.tasks.filter(t => t._id !== deletedTaskId)
        }));
  ...
  const getAllComments = async (taskId: string) => {
    await axios.get(`/comment/${taskId}`).then((res) => {
      const commentsWithUsers = res.data.data.map(async (comment: Comment) => {
        const userRes = await axios.get(`/user/${comment.userId}`);
        return { ...comment, user: {
